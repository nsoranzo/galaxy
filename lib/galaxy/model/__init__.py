...
class Job(JobLike, UsesCreateAndUpdateTime, Dictifiable, RepresentById):
    ...
    def update_hdca_update_time_for_job(self, update_time, sa_session):
        ...
        hdca_ids = [hdca.id for hdca in subq.all()]
        statement = HistoryDatasetCollectionAssociation.table.update() \
            .where(HistoryDatasetCollectionAssociation.table.c.id.in_(hdca_ids)) \
            .values(update_time=update_time)
        sa_session.execute(statement)
        statement = HistoryDatasetCollectionAssociation.table.update() \
